...
void Segment::removeTailSegment()
{
...
void Segment::addHeadSegment(Segment const& newHead)
{
...
void Segment::removeTailSegmentIfNotScored(Segment const& newHead)
{
...
void Segment::updateSegmentsIfSuccessfullMove(Segment const& newHead)
{
...
{
    if (Segment::isSegmentAtPosition(x, y) || isPositionOutsideMap(x,y)) {
        m_foodPort.send(std::make_unique<EventT<FoodReq>>());
        return;
    }
